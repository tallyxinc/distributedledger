...
- Peer organization org1 `org1` 
- Peer organization org2 `org2` 
...
- **peer0** (ex.: `peer0.org1.example.com`) with the anchor [peer](https://github.com/hyperledger/fabric/tree/release/peer) runtime
- **peer1** `peer1.org1.example.com` with the secondary peer
- **ca** `ca.org1.example.com` with certificate authority server [fabri-ca](https://github.com/hyperledger/fabric-ca)
- **api** `api.org1.example.com` with [fabric-rest](https://github.com/Altoros/fabric-rest) API server
- **www** `www.org1.example.com` with a simple http server to serve members' certificate files during artifacts generation and setup
- **cli** `cli.org1.example.com` with tools to run commands during setup
...
thus their `1000` ports are mapped to non conflicting `1000, 1001` ports on the host.
...
Generated crypto material of all members, block, network-config file and tx files are placed in shared `artifacts` folder on the host.
...
- org1 [http://localhost:1000/admin](http://localhost:1000/admin)
- org2 [http://localhost:1001/admin](http://localhost:1001/admin)
...
# members
./network.sh -m logs -m org1
./network.sh -m logs -m org2
```
Stop all:
```bash
./network.sh -m down
...
domain names `api.org1.example.com`, `peer1.org2.example.com` etc. However they can no longer discover each other within a local
...
```bash
export IP_ORDERER=54.235.3.243 IP1=54.235.3.231 IP2=54.235.3.232
```  
...
```bash
# organization org1 on their host
./network.sh -m generate-peer -o org1

# organization org2 on their host
./network.sh -m generate-peer -o org2
...
When the orderer is up, each member can start services on their hosts and their peers connect to the orderer to create channels. Note that in Fabric one member creates a channel and others join to it via a channel block file. 
...
for example for channel `org1-org2` member `org1` needs to start first to create the channel and serve the block file,
and then `org2` starts, downloads the block file and joins the channel. It's a good idea to order organizations in script arguments alphabetically, ex.: `ORG1=aorg ORG2=borg ORG3=corg` then the channels are named accordingly 
...
```bash
export DOMAIN=myapp.com ORG1=bar ORG2=baz
```  
...
It's also relatively straightforward to extend the scripts from the preset `ORG1` and `ORG2` to take an arbitrary 
